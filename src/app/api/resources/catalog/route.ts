  ...
  try {
    ...
    const [
      ...
      !resourceType || resourceType === "DATABASE"
        ? prisma.resourceDatabase.findMany({
            where: { isActive: true },
            select: {
              id: true,
              name: true,
      ...
      !resourceType || resourceType === "SERVER"
        ? prisma.resourceServer.findMany({
            where: { status: "ACTIVE" },
            select: {
              id: true,
              name: true,
      ...
      !resourceType || resourceType === "DEVICE"
        ? prisma.resourceDevice.findMany({
      ...
      !resourceType || resourceType === "VPN_NETWORK_ACCESS"
        ? prisma.vPNNetworkAccess.findMany({
            where: { status: "ACTIVE" },
            select: {
              id: true,
              profileName: true,
      ...
      !resourceType || resourceType === "CODE_REPOSITORY"
        ? prisma.codeRepository.findMany({
            where: { status: "ACTIVE" },
            select: {
              id: true,
              platform: true,
      ...
      !resourceType || resourceType === "PHYSICAL_ACCESS"
        ? prisma.physicalAccess.findMany({
            where: { status: "ACTIVE" },
            select: {
              id: true,
              location: true,
